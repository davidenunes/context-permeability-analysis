...
The analysis for each context switching probability span is as follows: 
```{r load-parameters}
...
**Note** I don't need custom breaks for the **persp plots**, its simpler to apply a logarithm to a set of n numbers, normalize them inside the plotting function, and multiply this by the maximum value in the dataset, this gets me the effect I'm looking for both the filling and contour lines.
...
#**************************************************************************
params <- "../data/context_permeability/regular_networks/param-space_2014-05-30_16:47:53experiment:0_.csv"
...
for(cs in c(0.25,0.5,0.75)){
  ...
  #gets too messy with permeability data 
  #perm_data <- dcast(perm_encounters, network.0.k + num.networks ~ variable, mean)
  #colnames(perm_data) <- c("k","num-networks", "avg-encounters")
  ...
  #add line for permeability
  #plot <- plot +  geom_point(data=perm_data, aes(x=as.factor(perm_data$k), y=perm_data$"avg-encounters", shape="permeability", color=as.factor(perm_data$"num-networks")),size = 3.5)
  #plot +  geom_line(data=perm_data, aes(x=as.factor(perm_data$k), y=perm_data$"avg-encounters",group=as.factor(perm_data$"num-networks"),linetype=as.factor(plot_data$"num-networks"),color=as.factor(perm_data$"num-networks")),size=1)   
